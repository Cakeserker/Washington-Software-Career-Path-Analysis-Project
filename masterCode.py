...
import os
import csv
import synonyms  #!!! THIS IS FOR TESTING
...
#-------------
# #!!! part of the quick testing   
# checkpoint_file = 'checkpoint.txt'
# def save_checkpoint(file_path):
#     with open(checkpoint_file, 'w') as f:
#         f.write(file_path)

# #!!! I DON'T THINK THIS SEGMENT IS NEEDED BUT IDK?
...
#     if not any(last_processed_file.endswith(f'cpadataset{i}.json') for i in range(1, upper_limit + 1)):
#         # If not, it means the checkpoint file is from a previous run
#         start_from_checkpoint = False
# else:
#     last_processed_file = None
#     start_from_checkpoint = False
    
# #!!! Original implementation of extracting json. Only extracts json10, can be used for quick testing
# for x in range(1, upper_limit + 1):
#     json_file_path = f'cpadataset{x}.json'

#     if start_from_checkpoint:
#         # Skip files until reaching the last processed file
#         if json_file_path != last_processed_file:
#             continue
#         else:
#             start_from_checkpoint = False
#     try:
...
#             last_processed_file = json_file_path
#             #save_checkpoint(last_processed_file)
